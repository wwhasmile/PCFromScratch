...
public class InternalDriveService(IInternalDriveRepository internalDriveRepository) : IInternalDriveService
{
    ...
    {
        await foreach (var internalDrive in _internalDriveRepository.GetInternalDrives(type, capacity))
    ...
    {
        var internalDrive = await _internalDriveRepository.GetInternalDrive(id);
