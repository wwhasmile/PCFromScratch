...
public class CoolerService(ICoolerRepository coolerRepository) : ICoolerService
{
    public async IAsyncEnumerable<CoolerDtoModel> GetCoolers(string? socket = null)
    {
        await foreach (var cooler in coolerRepository.GetCoolers(socket))
            yield return new(cooler.Id, cooler.Name, cooler.Link, cooler.Tdp, cooler.FanCount, cooler.Radius,
                cooler.Thickness, cooler.Speed, cooler.Height, cooler.Type, cooler.ImageUrl, cooler.MinPrice,
                cooler.MaxPrice);
    }
    ...
    public async Task<CoolerDtoModel?> GetCooler(Guid id)
    {
        ...
        return new(cooler.Id, cooler.Name, cooler.Link, cooler.Tdp, cooler.FanCount, cooler.Radius, cooler.Thickness,
